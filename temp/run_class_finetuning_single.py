...
def get_args():
    ...
    # 如果提供了 YAML 配置，则先加载为默认值，随后命令行参数可覆写
    if known_args.config is not None and os.path.isfile(os.path.expanduser(known_args.config)):
        config_path = os.path.expanduser(known_args.config)
        with open(config_path, 'r') as f:
            cfg = yaml.safe_load(f) or {}
        if not isinstance(cfg, dict):
            raise ValueError('YAML 配置需为字典（key-value）结构')
        allowed_keys = {a.dest for a in parser._actions}
        defaults_from_yaml = {k: v for k, v in cfg.items() if k in allowed_keys}
